...
`V(r) = U(r)/q = ∫[r→∞] E⃗ · dl⃗`, `[V] = V (volt) = J/C`
...
`V_A - V_B = W_A→B / q  →  W_A→B = q·(V_A - V_B)`
...
`|E_proj| = |V_A - V_B| / d_AB`, `d_AB = ‖r⃗_B - r⃗_A‖`
